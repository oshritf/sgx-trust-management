...
size_t ias_report_header_parser(void *ptr, size_t size, size_t nmemb, void *userdata) {
    ...
    if (parsed_fields == 1) {
        std::string iasreport_signature_str( p_iasreport_signature );
    ...
    if (parsed_fields == 1) {
        std::string p_iasreport_signing_certificate_str ( p_iasreport_signing_certificate );
...
)
{
    ...
    ias_report.report_body = string(ias_report_container.p_report);
