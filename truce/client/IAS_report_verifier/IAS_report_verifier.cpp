...
bool verify_cert_chain(X509 *ca_certX509, X509 *cert_chainX509, debug_print_func debug_print) {
    ...
    /*debug_print("Verification return code: %d\n", ret);

    if(ret == 0 || ret == 1) {
        debug_print("Verification result text: %s\n",
...
bool verify_IAS_report_signature(const IAS_report &ias_report, debug_print_func debug_print) {
    ...
    Base64Decode(ias_report.report_signature_base64.c_str(), &sigbuf, &siglen, debug_print);
    ...
    SHA256((unsigned char*) ias_report.report_body.c_str(), ias_report.report_body.length(), md);
...
bool extract_quote_from_IAS_report(const IAS_report &ias_report,
    ...
    size_t start_pos = ias_report.report_body.find(prefix) + strlen(prefix);
    if (start_pos == string::npos) {
        debug_print("Report body doesn't contain isvEnclaveQuoteBody\n");
        return false;
    }
    string tmp_body = ias_report.report_body.substr(start_pos);
...
#ifdef SIMULATE_IAS
    debug_print("******* Warning: Running in IAS simulation mode. Skipping signature verification.\n");
#else
